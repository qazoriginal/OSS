...
if [[ "$2" != "" ]];
then
	echo "usage: ./audit.sh [--help]" >&2
	exit
fi
case "$1" in 
	--help)
		echo "Этот сценарий позволяет регистрировать и анализировать события с помощью linux auditing system." >&2
		exit
		;;
esac
if [[ "$1" != "" ]];
...
echo -e "\nС помощью данной программы можно регистрировать и анализировать события с помощью linux auditing system.\n\nРазработчик: Панков Дмитрий Евгеньевич, группа Б20-505\n\n"
echo -e "Главное меню:\n"
PS3=$'\n> '
options=(
	...
	"Настройка подсистемы аудита")
		...
		case $opt in
            ...
            "Назад")
                break
                ;;
	    *) echo "Неправильная команда $REPLY" >&2
		;;
		esac
		done
        ;;
